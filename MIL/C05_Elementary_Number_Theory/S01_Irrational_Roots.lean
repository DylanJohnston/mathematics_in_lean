...
example {m n : ℕ} (coprime_mn : m.Coprime n) : m ^ 2 ≠ 2 * n ^ 2 := by
  ...
  have two_dvd_m : 2 ∣ m := by
    ...
    rw [sqr_eq, Nat.Prime.dvd_mul Nat.prime_two]
    left; norm_num
  ...
  have : 2 * (2 * k ^ 2) = 2 * n ^ 2 := by
    ...
    ring
  have : 2 * k ^ 2 = n ^ 2 := by
    exact (mul_right_inj' two_ne_zero).mp this
...
example {m n p : ℕ} (coprime_mn : m.Coprime n) (prime_p : p.Prime) : m ^ 2 ≠ p * n ^ 2 := by
  ...
  have p_dvd_m : p ∣ m := by
    ...
    rw [pow_eq, Nat.Prime.dvd_mul prime_p]
    left; norm_num
  ...
  have p_dvd_n : p ∣ n := by
    apply Nat.Prime.dvd_of_dvd_pow prime_p
    rw [this.symm]
    rw [Nat.Prime.dvd_mul prime_p]
    left; norm_num
  have : p ∣ m.gcd n := by
    apply Nat.dvd_gcd p_dvd_m p_dvd_n
  have : p ∣ 1 := by
    rwa [coprime_mn] at this
  have p_ge_1 : p > 1 := by apply Nat.Prime.two_le prime_p
  have p_leq_1 : p ≤ 1 := by
    apply Nat.le_of_dvd Nat.one_pos this
  linarith
...
example {m n p : ℕ} (nnz : n ≠ 0) (prime_p : p.Prime) : m ^ 2 ≠ p * n ^ 2 := by
  ...
  have eq2 : (p * n ^ 2).factorization p = 2 * n.factorization p + 1 := by
    ...
    simp [Nat.Prime.factorization' prime_p]
    ring
    apply Prime.ne_zero
    exact Nat.prime_iff.mp prime_p
    assumption
...
example {m n k r : ℕ} (nnz : n ≠ 0) (pow_eq : m ^ k = r * n ^ k) {p : ℕ} :
  ...
  have eq2 : ((r + 1) * n ^ k).factorization p =
      ...
      rw [factorization_mul', factorization_pow']; ring
